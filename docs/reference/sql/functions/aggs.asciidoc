...
In case of COUNT(*) or COUNT(<numeric_value>), _all_ values are considered (including `null` or missing ones). In case of
COUNT(<field_name>) `null` values are not considered.
COUNT(<field_name>) and COUNT(ALL <field_name>) are equivalent. `ALL` can only be used with a field name.
